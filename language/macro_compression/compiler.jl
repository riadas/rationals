...
function expand(lang_str)
    ...
    if !occursin("function RN_op(", lang_str) && occursin("RN_op", old_lang_str)
        lang_str = "$(lang_str)\n\n$(RN_op_defn)"
    end
...
function add_RN(x::RN, y::RN)
   cm = common_multiple(arg1.denominator, arg2.denominator)
   scaled_arg1 = scale(arg1, cm / arg1.denominator)
   scaled_arg2 = scale(arg2, cm / arg2.denominator)
   RN(eval(:+)(scaled_arg1.numerator, scaled_arg2.numerator), cm)
end

function subtract_RN(x::RN, y::RN)
   cm = common_multiple(arg1.denominator, arg2.denominator)
   scaled_arg1 = scale(arg1, cm / arg1.denominator)
   scaled_arg2 = scale(arg2, cm / arg2.denominator)
   RN(eval(:-)(scaled_arg1.numerator, scaled_arg2.numerator), cm)
...
function compare_RN(x::RN, y::RN, op::Symbol)
   cm = common_multiple(arg1.denominator, arg2.denominator)
   scaled_arg1 = scale(arg1, cm / arg1.denominator)
   scaled_arg2 = scale(arg2, cm / arg2.denominator)
   RN(eval(op)(scaled_arg1.numerator, scaled_arg2.numerator), cm)
...
function translate_spec_to_type_system(spec, helpers_derivable=false)
    ...
    elseif spec["relate"] == "false"
        ...
        lang_template = "$(lang_template)\n$(split_combine_post_relate_template)"
        if !helpers_derivable && spec["add_op"] != "NullNumber"
            lang_template = "$(lang_template)\n\n$(helper_template)"
        end
...
# lang_str = ""
# open("language/macro_compression/compressed/8_rational_arithmetic_ungrounded_language_compressed.jl", "r") do f 
...
for language_name_ in base_language_names 
...
# TODO 
# - implement the variant version of this compiler -- DONE
# - implement the length function -- DONE
...
function generate_type_system(language_index, helpers_derivable=false; return_init=false)
    ...
    println("LANGUAGE NAME: $(language_names_pretty[language_index])")
    println("\nSTART STRING\n")
...
function generate_all_type_systems(helpers_derivable=false; return_init=false)
    d = Dict()
    for i in 1:length(language_names_pretty[1:10])
...
for helpers_derivable in [false, true]
    ...
    for k in keys(d)
        result = d[k] 
        folder_names = readdir("language/macro_compression/auto_generated/helpers_derivable")
        for i in 1:length(folder_names)
            folder_name = folder_names[i]
            helper_derivable_folder = helpers_derivable ? "helpers_derivable" : "helpers_not_derivable" 
            open("language/macro_compression/auto_generated/$(helper_derivable_folder)/$(folder_name)/$(k)", "w+") do f 
                write(f, result[i])
            end
        end
    end
