...
function halve()
    [halve1] # INCOMPLETE: RationalNumber(1, 2)
end

function halve(n::NaturalNumber)
    [halve2] # INCOMPLETE: RationalNumber(n, 2)
end

function halve(rn::RationalNumber)
    [halve3] # INCOMPLETE: RationalNumber(rn.numerator, rn.denominator * 2) # halve_obj(PhysicalObject(rn)).size
end

function double(rn::RationalNumber)
     [double] # INCOMPLETE: RationalNumber(rn.numerator*2, rn.denominator)
end

function divide(n::NaturalNumber)
    [divide1] # INCOMPLETE: RationalNumber(1, n)
end

function divide(rn::RationalNumber, n::NaturalNumber)
    [divide2] # INCOMPLETE: RationalNumber(rn.numerator, rn.denominator * n) # split_obj(PhysicalObject(rn), n).size
end

function multiply(rn::RationalNumber, n::NaturalNumber)
    [multiply] # INCOMPLETE: RationalNumber(rn.numerator*n, rn.denominator) # combine_obj(PhysicalObject(rn), n).size
end
...
function divide(n::NaturalNumber, m::NaturalNumber)
    ...
    # RationalNumber(n, m) 
    [divide3] # INCOMPLETE: cast_NN((divide_obj(PhysicalObject(1), n, m)).size)
end
...
function multiply(arg1::RationalNumber, arg2::RationalNumber)
    [multiply_op] # INCOMPLETE: divide(multiply(arg1, arg2.numerator), arg2.denominator)
end

function divide(arg1::RationalNumber, arg2::RationalNumber)
    [divide_op] # INCOMPLETE: divide(multiply(arg1, arg2.denominator), arg2.numerator)
end
...
infinite_divisibility_space = [infinite_divisibility_space] # start: fine
infinite_divisibility_number = [infinite_divisibility_number] # start: coarse
infinite_divisibility_weight = [infinite_divisibility_weight] # start: coarse